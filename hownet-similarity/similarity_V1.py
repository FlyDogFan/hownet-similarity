...
if __name__ == '__main__':
    ...
    #对于每一个协陪义动词，生成一个result序号.txt文件，存放“当前协陪义动词--glossary.dat中动词    相似度”
    bt_xiepeiyiVerb.dic中有多少行，生成多少个result序号.txt文件
    '''
    generatePlabel = True
    SIMILARITY = True
    ...
    if SIMILARITY:
        ...
        resultFile = './result/result1.txt'
        resultdic = {}
        writer = open(resultFile, 'wt', encoding='utf-8')
        count = 2
        for line in lines:
            if '-1' == line and resultdic!={}:
                ...
                resultdic.clear()
                resultdic = {}
                resultFile = re.sub('\d+?',str(count),resultFile,count = 1)
                count = count + 1
                writer = open(resultFile, 'wt', encoding='utf-8')
            else:
                l = line.split()
